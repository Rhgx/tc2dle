...
export function WeaponGame({ weapons, status }: WeaponGameProps) {
  ...
  useEffect(() => {
    ...
    setMessage("");
    setGuesses(readCachedGuesses(target.name, weapons));
    setLoadedAnswer(target.name);
  }, [target, weapons]);

  useEffect(() => {
    if (!loadedAnswer || loadedAnswer !== target?.name) return;
    writeCachedGuesses(loadedAnswer, guesses);
...
function readCachedGuesses(answer: string, weapons: Weapon[]): GuessEntry[] {
  try {
    const parsed = readCachedGuessNames("weapon", answer) as unknown;
    if (!Array.isArray(parsed)) return [];

    const byName = new Map(weapons.map((weapon) => [weapon.name, weapon]));
    return parsed
      .filter((name): name is string => typeof name === "string")
      .map((name, index) => byName.get(name) ? { id: `weapon-${answer}-${name}-${index}`, weapon: byName.get(name)!, revealStage: 6 } : null)
...
function writeCachedGuesses(answer: string, guesses: GuessEntry[]) {
  writeCachedGuessNames("weapon", answer, guesses.map((entry) => entry.weapon.name));
}
